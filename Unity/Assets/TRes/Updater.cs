...
namespace Tuner.Resource
{
		...
		{	
				...
				UpdateVersionInfo mNetVersionInfo = null;
				UpdateVersionInfo mLocalVersionInfo = null;
				...
				void NetInfoCallBack (string url, System.Object value, E_Resource_ErrorCode result, string message)
				{
						if (result == E_Resource_ErrorCode.Success) {
								//build
								BuildResourcesInfo (ref mNetVersionInfo, value.ToString ());
								compare ();
						} else {
				...
				void LocalInfoCallBack (string url, System.Object value, E_Resource_ErrorCode result, string message)
				{
						if (result == E_Resource_ErrorCode.Success) {
								//build
								BuildResourcesInfo (ref mLocalVersionInfo, value.ToString ());
								compare ();
						} else {
				...
				void BuildResourcesInfo (ref UpdateVersionInfo ver, string value)
				{
						ver = new UpdateVersionInfo ();
						ver.Deserialize (value);
				}
